// ============================================================
// OLLY HENSON COACHING — Regulate for Relief 6-Week Tracker
// ============================================================
//
// SHEET SETUP — tabs required:
//   - Baseline Responses (linked via Zapier)
//   - Weekly Responses   (linked via Zapier)
//   - Final Responses    (linked via Zapier)
...
//   Pain, Energy, Anxiety, Calmness, Sleep,
//   Depressive Thoughts, Focus, Mood, Stress Resilience,
//   Meditations this week, Wins this week, Anything you need help with?
//
...
//
// SCRIPT SETUP:
//   1. Fill in CONFIG values below if anything changes
...
const CONFIG = {
  ...
  RESEND_API_KEY:    're_N6dLAFC4_GSALj37r7FdVFCw6SAsEHBRC',
...
function processFinalRow(row) {
  ...
  const baselineScores = extractScores(baseline);
  const currentScores = extractScores(row); // score columns match COLS positions
...
function updateSummaryDashboard() {
  ...
  dashSheet.getRange(1, 1).setValue('Regulate for Relief — Results Summary');
  ...
  // Border around data
  const totalRows = 3 + METRICS.length;
...
function sendEmail(to, subject, htmlBody, plainText) {
  UrlFetchApp.fetch('https://api.resend.com/emails', {
    ...
    payload: JSON.stringify({
      from: 'Regulate for Relief Program <olly@ollyhenson.com>',
      to: [to],
      subject: subject,
      html: htmlBody,
      text: plainText || '',
...
function sendBaselineConfirmation(email, name) {
  const subject = `Regulate for Relief starts now`;

  const html = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>Your baseline scores have been recorded and you've officially started the six weeks of Regulate for Relief.</p>
    <p>Your first check-in will arrive in your inbox in seven days.</p>
    <p>Just do your best to complete the meditation in the Classroom twice per day.</p>
    ...
    <p>${link(CONFIG.SHARE_BASE_URL + '?type=started&text=' + encodeURIComponent('I\'ve just started the Regulate for Relief program — excited to get going!'), 'Let us know that you\'ve started here →')}</p>
    <p>To your health, happiness and success.<br>Olly</p>
  `);

  sendEmail(email, subject, html, 'Your baseline scores have been recorded. You\'ve officially started the six weeks of Regulate for Relief.');
  sendBaselineNotificationToOlly(email, name);
}

function sendBaselineNotificationToOlly(email, name) {
  const subject = `${name} has started Regulate for Relief`;
  const startedText = `${firstName(name)} starts Regulate for Relief today. You've got this. Keep us posted with how you're doing.`;
  const html = wrapHtml(`
    <p>${name} has just started their six weeks of Regulate for Relief.</p>
...
function sendWeeklyFormEmail(email, name, weekNumber) {
  const subject = `Your week ${weekNumber} check-in — Regulate for Relief Program`;
...
function sendFinalFormEmail(email, name) {
  const subject = `Your final check-in — Regulate for Relief Program`;

  const html = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>You've made it to your final check-in for Regulate for Relief — well done for completing the six weeks.</p>
...
function sendProgressReport(email, name, weekNumber, baseline, current, meditations, wins, needs, isFinal, howFeel, biggestPos, recommend, improvements) {
  ...
  const clientSubject = isFinal
    ? `Your final progress report — Regulate for Relief Program`
    : `Your week ${weekNumber} progress report — Regulate for Relief Program`;

  const clientHtml = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>${isFinal ? 'Congratulations on completing the Regulate for Relief Program!<br><br>Here are your final results:' : `Here is your Week ${weekNumber} progress report.`}</p>
  ...
  const ollyHtml = wrapHtml(`
    ...
    <p>Improved: <strong>${improved}/9</strong> &nbsp;|&nbsp; Declined: <strong>${declined}/9</strong> &nbsp;|&nbsp; Status: <strong>${statusLabel}</strong></p>
    ${!isFinal ? `<p><strong>Meditations this week:</strong> ${meditations}</p>` : ''}
...
function testReport() {
  sendProgressReport(
    ...
    5, 'Feeling more focused', 'More support on meditation technique',
    false
  );
}
...
function testFinalReport() {
  sendProgressReport(
    ...
    6,
    { PAIN:8, ENERGY:2, ANXIETY:7, CALMNESS:3, SLEEP:4, DEPRESSIVE:8, FOCUS:5, MOOD:4, RESILIENCE:0 },
    { PAIN:4, ENERGY:7, ANXIETY:3, CALMNESS:7, SLEEP:7, DEPRESSIVE:3, FOCUS:8, MOOD:8, RESILIENCE:6 },
    6, 'Back to walking every day', '',
    true,
    'I feel like myself again. The chronic pain is mostly gone and I have energy to live a full life.',
    'Realising my nervous system was the root cause of everything. Once I understood that, everything started shifting.',
    'Without question. I only wish I had found this programme years ago.',
    'Nothing — the programme structure was exactly what I needed.'
  );
}

function testPoorFinalReport() {
  sendProgressReport(
    'olly@ollyhenson.com',
    'Olly Henson',
    6,
    { PAIN:7, ENERGY:3, ANXIETY:8, CALMNESS:2, SLEEP:3, DEPRESSIVE:7, FOCUS:4, MOOD:3, RESILIENCE:2 },
    { PAIN:6, ENERGY:4, ANXIETY:7, CALMNESS:3, SLEEP:3, DEPRESSIVE:8, FOCUS:3, MOOD:3, RESILIENCE:2 },
    3, '', 'Still struggling with consistency and finding the practice hard to stick to',
    true,
    'I still feel quite up and down if I am honest. Some days are better than others but I haven\'t noticed a huge shift yet.',
    'I suppose just knowing there is something I can do about how I feel.',
    'Maybe — I think I need more support to make it work for me.',
    'More check-ins or support along the way would have helped me stay on track.'
  );
}
