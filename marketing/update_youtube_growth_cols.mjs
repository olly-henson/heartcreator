...
const res = await fetch(`https://api.notion.com/v1/databases/${DB_ID}`, {
  ...
  body: JSON.stringify({
    properties: {
      "Subscriber Growth (%)":  { number: { format: "percent" } },
      "View Growth (%)":        { number: { format: "percent" } },
      "Watch Time Growth (%)":  { number: { format: "percent" } },
    },
  }),
});
console.log(res.ok ? "✅ Growth % columns added" : `❌ ${JSON.stringify(await res.json())}`);
