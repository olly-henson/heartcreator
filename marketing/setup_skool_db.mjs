...
const DB_ID = env["NOTION_SKOOL_MEMBERS_DB_ID"];
...
// Remove all non-title columns
const toDelete = Object.keys(existing.properties).filter(c => !["Period", "Month"].includes(c));
const delProps = {};
...
// Rename title column to Month if needed
...
const columns = [
  ...
  ["New Members",        { number: { format: "number" } }],
...
console.log("\n✅ Skool Community Growth database ready");
