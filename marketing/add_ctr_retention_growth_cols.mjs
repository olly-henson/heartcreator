...
const res = await fetch(`https://api.notion.com/v1/databases/${env["NOTION_YOUTUBE_DB_ID"]}`, {
  ...
  body: JSON.stringify({
    properties: {
      "CTR Growth (%)":       { number: { format: "percent" } },
      "Retention Growth (%)": { number: { format: "percent" } },
    },
  }),
});
console.log(res.ok ? "✅ CTR Growth and Retention Growth columns added" : `❌ ${JSON.stringify(await res.json())}`);
