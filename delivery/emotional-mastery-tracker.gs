// ============================================================
// OLLY HENSON COACHING — Emotional Mastery 6-Week Tracker
// ============================================================
//
...
//   Full Name, Email,
//   How easy is it for you to feel your chosen emotion in meditation or when visualizing?,
//   How easy is it for you to maintain the feeling of your chosen emotion in meditation or when visualizing?,
//   How easy is it for you to tap into your chosen emotion at any point in the day?
//
...
//
// SCRIPT SETUP:
//   1. Fill in CONFIG values below (Resend API key)
...
const CONFIG = {
  ...
  WEEKLY_FORM_URL:   'https://docs.google.com/forms/d/e/1FAIpQLSfQhiJXepfITgQcNlSwHuL6QVqWXGb4SoymVBAdx9QGBNV1ow/viewform?usp=header',
  FINAL_FORM_URL:    'https://docs.google.com/forms/d/e/1FAIpQLSfn0yoAmzWXZzAhF9DSDL2dKv2nfyDsJFmhIeVg95HBVo0Z7w/viewform?usp=header',
...
const METRICS = [
  { key: 'FEEL',     label: 'Feeling in Meditation', direction: 'up' },
  { key: 'MAINTAIN', label: 'Maintaining the Feeling', direction: 'up' },
  { key: 'TAP_IN',   label: 'Tapping In During the Day', direction: 'up' },
];
...
function addToMaster(name, email, startDate, endDate, nextCheckIn, scores) {
  ...
  if (!masterSheet) {
    ...
    masterSheet.appendRow([
      ...
      'Feeling in Meditation', 'Maintaining the Feeling', 'Tapping In During the Day',
      'Total Meditations to Date', 'Wins This Week', 'Needs Help With'
    ]);
  }

  masterSheet.appendRow([
    name, email, startDate, endDate, nextCheckIn, '', 0,
    scores.FEEL, scores.MAINTAIN, scores.TAP_IN,
    '', '', ''
  ]);
...
function updateMaster(email, weekNumber, scores, baseline, nextCheckIn, meditations, wins, needs) {
  ...
  const status = statusImproved >= 2 ? '🟢 Going Well'
               : statusImproved >= 1 ? '🟡 Going Okay'
               : statusDeclined >= 2 ? '🔴 Needs Help'
               : '🟠 Might Need Help';
...
function updateSummaryDashboard() {
  ...
  dashSheet.getRange(1, 1).setValue('Emotional Mastery — Results Summary');
...
function sendEmail(to, subject, htmlBody, plainText) {
  UrlFetchApp.fetch('https://api.resend.com/emails', {
    ...
    payload: JSON.stringify({
      from: 'Emotional Mastery Program <olly@ollyhenson.com>',
      to: [to],
      subject: subject,
      html: htmlBody,
      text: plainText || '',
...
function sendBaselineConfirmation(email, name) {
  const subject = `Emotional Mastery starts now`;

  const html = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>Your baseline scores have been recorded and you've officially started the six weeks of Emotional Mastery.</p>
    <p>Your first check-in will arrive in your inbox in seven days.</p>
    <p>Each day, do your best to complete the Emotional Mastery Meditation and to self-regulate throughout your day.</p>
    ...
    <p>${link(CONFIG.SHARE_BASE_URL + '?type=started&text=' + encodeURIComponent('I\'ve just started the Emotional Mastery program — excited to get going!'), 'Let us know that you\'ve started here →')}</p>
    <p>To creating and living your life with confidence and ease.<br>Olly</p>
  `);

  sendEmail(email, subject, html, 'Your baseline scores have been recorded. You\'ve officially started the six weeks of Emotional Mastery.');
  sendBaselineNotificationToOlly(email, name);
}

function sendBaselineNotificationToOlly(email, name) {
  const subject = `${name} has started Emotional Mastery`;
  const startedText = `${firstName(name)} starts Emotional Mastery today. You've got this. Keep us posted with how you're doing.`;
  const html = wrapHtml(`
    <p>${name} has just started their six weeks of Emotional Mastery.</p>
...
function sendWeeklyFormEmail(email, name, weekNumber) {
  const subject = `Your week ${weekNumber} check-in — Emotional Mastery Program`;
...
function sendFinalFormEmail(email, name) {
  const subject = `Your final check-in — Emotional Mastery Program`;

  const html = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>You've made it to your final check-in for Emotional Mastery — well done for completing the six weeks.</p>
...
function sendProgressReport(email, name, weekNumber, baseline, current, meditations, wins, needs, isFinal, howFeel, biggestPos, recommend, improvements) {
  ...
  const statusLabel = improved >= 2 ? '🟢 Going Well'
                   : improved >= 1 ? '🟡 Going Okay'
                   : declined >= 2 ? '🔴 Needs Help'
                   : '🟠 Might Need Help';
  ...
  const clientSubject = isFinal
    ? `Your final progress report — Emotional Mastery Program`
    : `Your week ${weekNumber} progress report — Emotional Mastery Program`;

  const clientHtml = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>${isFinal ? 'Congratulations on completing the Emotional Mastery Program!<br><br>Here are your final results:' : `Here is your Week ${weekNumber} progress report.`}</p>
  ...
  const ollyHtml = wrapHtml(`
    ...
    ${declined >= 2 ? '<p style="color:#c62828;"><strong>⚠️ FLAGGED: 2 or more metrics declined — may be worth a personal check-in.</strong></p>' : ''}
  `);
...
function testReport() {
  sendProgressReport(
    ...
    'Olly Henson',
    3,
    { FEEL: 3, MAINTAIN: 2, TAP_IN: 2 },
    { FEEL: 6, MAINTAIN: 5, TAP_IN: 5 },
    5, 'Felt my emotion strongly for the first time during meditation', 'Help staying in the feeling for longer',
    false
  );
}
...
function testFinalReport() {
  sendProgressReport(
    ...
    6,
    { FEEL: 3, MAINTAIN: 2, TAP_IN: 2 },
    { FEEL: 9, MAINTAIN: 8, TAP_IN: 8 },
    14, 'Can drop into joy whenever I choose', '',
    true,
    'I genuinely feel like I have a superpower. I can access joy and gratitude on demand.',
    'Realising I had the ability to feel this way all along — I just needed to practise it.',
    'Without question. This has changed how I experience every single day.',
