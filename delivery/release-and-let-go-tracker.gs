// ============================================================
// OLLY HENSON COACHING — Release & Let Go 30-Day Tracker
// ============================================================
//
...
//   Email, Full Name,
//   Sense of Lightness, Energy, Calmness, Clarity of Mind
//
...
//
// SCRIPT SETUP:
//   1. Fill in CONFIG values below (form URLs, classroom URL)
...
const CONFIG = {
  ...
  WEEKLY_FORM_URL:   'https://docs.google.com/forms/d/e/1FAIpQLSf9kd9LSLZvcYEWQ8Xmb-hBOK3r4VF5SwoDj9BpXfU8eVVtMw/viewform?usp=header',
  FINAL_FORM_URL:    'https://docs.google.com/forms/d/e/1FAIpQLSdApQlsGR29zyI1k72dLzRK9O74PzaerQ7OlP2SGimmJVWlpw/viewform?usp=header',
  SHARE_BASE_URL:    'https://share.ollyhenson.com',
  SKOOL_PROFILE_URL: 'https://www.skool.com/@olly-henson',
  CLASSROOM_URL:     'YOUR_SKOOL_CLASSROOM_URL',
...
const METRICS = [
  { key: 'LIGHTNESS', label: 'Sense of Lightness', direction: 'up' },
  { key: 'ENERGY',    label: 'Energy',              direction: 'up' },
  { key: 'CALMNESS',  label: 'Calmness',            direction: 'up' },
  { key: 'CLARITY',   label: 'Clarity of Mind',     direction: 'up' },
];
...
function addToMaster(name, email, startDate, endDate, nextCheckIn, scores) {
  ...
  masterSheet.appendRow([
    ...
    scores.LIGHTNESS, scores.ENERGY, scores.CALMNESS, scores.CLARITY,
    '', '', ''
  ]);
...
function updateMaster(email, weekNumber, scores, baseline, nextCheckIn, meditations, wins, needs) {
  ...
  const status = statusImproved >= 3 ? '🟢 Going Well'
               : statusImproved >= 2 ? '🟡 Going Okay'
...
function updateSummaryDashboard() {
  ...
  dashSheet.getRange(1, 1).setValue('Release & Let Go — Results Summary');
  ...
  dashSheet.setColumnWidth(1, 185);
...
function sendEmail(to, subject, htmlBody, plainText) {
  UrlFetchApp.fetch('https://api.resend.com/emails', {
    ...
    payload: JSON.stringify({
      from: 'Release & Let Go Program <olly@ollyhenson.com>',
      to: [to],
      subject: subject,
      html: htmlBody,
      text: plainText || '',
...
function sendBaselineConfirmation(email, name) {
  const subject = `Release & Let Go starts now`;

  const html = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>Your baseline scores have been recorded and you've officially started the four weeks of Release & Let Go.</p>
    <p>Your first check-in will arrive in your inbox in seven days.</p>
    <p>Each day, do your best to complete the Release & Let Go daily meditation and to self-regulate throughout your day.</p>
    ...
    <p>${link(CONFIG.SHARE_BASE_URL + '?type=started&text=' + encodeURIComponent('I\'ve just started the Release & Let Go program — excited to get going!'), 'Let us know that you\'ve started here →')}</p>
    <p>To creating and living your life with confidence and ease.<br>Olly</p>
  `);

  sendEmail(email, subject, html, 'Your baseline scores have been recorded. You\'ve officially started the four weeks of Release & Let Go.');
  sendBaselineNotificationToOlly(email, name);
}

function sendBaselineNotificationToOlly(email, name) {
  const subject = `${name} has started Release & Let Go`;
  const startedText = `${firstName(name)} starts Release & Let Go today. You've got this. Keep us posted with how you're doing.`;
  const html = wrapHtml(`
    <p>${name} has just started their four weeks of Release & Let Go.</p>
...
function sendWeeklyFormEmail(email, name, weekNumber) {
  const subject = `Your week ${weekNumber} check-in — Release & Let Go Program`;
...
function sendFinalFormEmail(email, name) {
  const subject = `Your final check-in — Release & Let Go Program`;

  const html = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>You've made it to your final check-in for Release & Let Go — well done for completing the four weeks.</p>
...
function sendProgressReport(email, name, weekNumber, baseline, current, meditations, wins, needs, isFinal, howFeel, biggestPos, recommend, improvements) {
  ...
  const statusLabel = improved >= 3 ? '🟢 Going Well'
                   : improved >= 2 ? '🟡 Going Okay'
  ...
  const clientSubject = isFinal
    ? `Your final progress report — Release & Let Go Program`
    : `Your week ${weekNumber} progress report — Release & Let Go Program`;

  const clientHtml = wrapHtml(`
    <p>Hi ${firstName(name)},</p>
    <p>${isFinal ? 'Congratulations on completing the Release & Let Go Program!<br><br>Here are your final results:' : `Here is your Week ${weekNumber} progress report.`}</p>
  ...
  const ollySubject = isFinal
    ? `[Final report] ${name} — 30-day summary`
...
function testReport() {
  sendProgressReport(
    ...
    'Olly Henson',
    2,
    { LIGHTNESS: 3, ENERGY: 4, CALMNESS: 3, CLARITY: 4 },
    { LIGHTNESS: 6, ENERGY: 6, CALMNESS: 6, CLARITY: 7 },
    5, 'Feeling lighter than I have in years', 'More support on the journaling practice',
    false
  );
}
...
function testFinalReport() {
  sendProgressReport(
    ...
    4,
    { LIGHTNESS: 3, ENERGY: 4, CALMNESS: 3, CLARITY: 4 },
    { LIGHTNESS: 8, ENERGY: 8, CALMNESS: 8, CLARITY: 9 },
    18, 'Felt completely free for the first time', '',
    true,
    'I feel like the weight of the past has genuinely lifted. I didn\'t think that was possible.',
    'Realising the past doesn\'t have to define me. That was everything.',
