...
**The safe-export canvas for an About Page tile is each section's own natural full height, never a fixed clip like 1400×790.** Force-clipping a `page.screenshot({clip:{...height:790}})` cut a taller section off mid-card with only 6px of bottom margin left — confirmed by measuring the exported PNG's actual content bounding box in Python/PIL and finding it nearly touching the canvas edge. The real working budget across this file's five shipped tiles is each section's own natural `boundingBox().height`, which lands in a ~915–950px band once a section is properly composed (not a fixed number — verify against a proven-good section, not a fixed target). Take the screenshot via `elementHandle.screenshot()` (captures the true natural height) rather than a hard-coded `clip`.

**When asked to enlarge a headline "as big as" one already-finished section while keeping the image in the same safe zone, the image must shrink to make room — verify with the actual measured height, not by eye.** Matching a longer/bigger headline's font-size to a shorter reference headline can add 2–3x more line-height (more words wrap to more lines at the same size), consuming budget the image needs. The reliable loop: set the new headline size, measure the section's natural height via Playwright `boundingBox()`, and if it's over the proven band, narrow the image frame's `max-width` (not the headline) until height lands back in range — repeat in ~50-100px steps rather than guessing one value. A near-square or portrait source image (aspect ratio near 1:1, e.g. 549×570) blows the height budget far faster than a landscape one (e.g. 1440×836) at the same frame width, because width increases translate almost 1:1 into height increases.

**Verify "even margins" by measuring the actual content bounding box in the exported PNG, not by eye in a screenshot.** A card that looked centered still had a 6px bottom margin against ~140px on the other three sides in one case, and 368px left/right against a much wider gutter in another (caused by a card's own `max-width` sitting narrower than its frame's padded content box, double-centering it). Both were only caught by scripting a bounding-box scan in Python/PIL for near-white pixels and comparing left/right/top/bottom margins numerically — do this any time "make it bigger"/"resize" work is done on one of these tiles, not just once at the end.

**A card whose own `max-width` is narrower than its frame's padded content box gets double-centered, producing uneven margins (wider on left/right than top/bottom, or vice versa).** Fix: set the card to `max-width:none; width:100%;` so it fills the frame's content box exactly, and let the *frame's own padding* (currently 32px in `.apt-skool-page-frame`) be the only source of margin — don't let two nested elements each contribute their own centering gap.

**When Olly provides a real screenshot of the *entire* card (photo + white text panel baked into one image, not just a photo crop), replace the whole `.apt-skool-card` content with a single `<img>` sized to the source's real aspect ratio (`object-fit:cover`), not the source photo dropped into the old separate photo-slot + live-text-body structure.** Keeping the old structure while swapping only the photo slot's `src` would render the real screenshot's own baked-in text *and* the separately-built live text underneath it — a visible duplicate. This is a deliberate trade: the title/description/progress-bar text on that tile is no longer independently editable via markup once this swap is made, only by editing the source PNG — confirm that trade is wanted before doing it (asked and confirmed 2026-08-30).

**To force one specific manual line-break in a headline that's otherwise auto-packed by the reverse-pyramid `packHeadline()` script, opt that one `<h2>` out rather than fighting the packer.** Add a `data-manual-break` attribute to the element and change the packer's query to `.apt-h2:not([data-manual-break])`, then hand-place `<br>` tags at the exact desired breaks. Don't use `&nbsp;`-gluing for this (see the mid-word-break bug documented above) and don't try to nudge the packer's width budget to coincidentally land a break where you want it — that's fragile and breaks again on the next resize/edit.

**The Artifact preview sandbox blocks the page from *triggering* a file download, but a plain `<img>` element with a data-URI `src` still supports the browser's native right-click "Save image as..." — use this to hand over a real exportable PNG.** Publish a minimal page (title, one instruction line, one `<img>`) with the target PNG base64-embedded; there is no working `<a download>` or script-driven save in this sandbox, so don't attempt one. Rebuild and republish this tiny wrapper page every time the underlying export PNG changes, using the same file path so the link stays stable.
...
- **Never export an About Page tile with a hard-coded clip height (e.g. 1400×790)** — the safe canvas is each section's own natural `boundingBox().height`; a fixed clip silently cut a card off with 6px of bottom margin left. Screenshot the element directly and measure against a proven-good section instead.
- **"Even margins" must be verified with an actual pixel bounding-box measurement (Python/PIL near-white scan), not eyeballed from a screenshot** — a double-centered card (its own `max-width` narrower than its frame's padded content box) produced visibly uneven left/right vs top/bottom margins that looked "close enough" until measured.
- **A card narrower than its frame's content box double-centers** — set the card to `max-width:none; width:100%;` so only the frame's own padding creates the margin, not two nested centering gaps.

---

## Changelog

**2026-08-30 — Post-session feedback review (about-page-tiles.html — headline/image resize + real-screenshot swaps across all 5 tiles)**
- New Variant section additions (static product tile export): documented the correct export-height rule (natural `boundingBox().height`, never a fixed clip — a fixed 790px clip cut a card off with 6px of bottom margin left, caught only after measuring the exported PNG's actual content bounding box in Python/PIL); the headline-vs-image tradeoff when matching one section's headline size to another (a longer headline eats more of the shared height budget, so the image frame must narrow to compensate — verified iteratively via `boundingBox()` measurement, not guessed); the double-centering bug (`.apt-skool-card`'s own `max-width` narrower than its frame's padded content box produces uneven margins) and its fix (`max-width:none; width:100%`); the correct approach for swapping in a real "whole card" screenshot (photo+text baked into one image) — replace the entire card's content with a single `<img>`, not just the photo slot's `src`, or the live-built text renders duplicated underneath; the `data-manual-break` attribute pattern for opting one headline out of the auto-packer to hand-place a specific line break; and the right-click-save-image workaround for handing over a real downloadable PNG through the Artifact sandbox (data-URI `<img>`, no `<a download>` or script-driven save).
- Gotchas Log: added four entries condensing the fixed-clip-height bug, the measure-don't-eyeball-margins rule, and the double-centering card fix into scannable form.
- No rules removed — all additions this round extend the existing static-tile-export variant (added 2026-08-28) with lessons from actually resizing and re-exporting all 5 shipped tiles this session, none of which contradicted the prior entry.
